...
The main effects plot is a visualization of the summary above. It shows the impact on the expected avereage response resulting from a change in level, e.g from 1 to -1, for each factor.
...
In accordance with what we saw in the summary, it appears that the strongest interaction is between factors A and C, i.e the height in which the bottle was dropped and the size of the bottle. Additionally, there seems to be some interaction between factors C and D, although it may not be significant.
...
ons such as solving linear systems, or for sparse matrix computations.
